...
class GaussSeidel extends Component
{
    constructor(props) {
      ...
      GaussSeidel = () => {
        ...
        const error = (xm_new, xm_old) => Math.abs((xm_new - xm_old) / xm_new)*100
        var iteration = 0
        while(iteration<1 || checkerror == true){
            ...
            for(var i=0;i<window.$A4.length;i++){
                window.$xnew4[i]=window.$B4[i]
                for(var j=0;j<window.$A4.length;j++){
                    if(i!=j){
                      if(i>j){
                        window.$xnew4[i]-=(window.$A4[i][j]*window.$xnew4[j])
                      }
                      else{
                ...
                // if(i==0){
                //     window.$arr_x1.push(window.$xnew4[i].toFixed(6))
                // }else if(i==1){
                //     window.$arr_x2.push(window.$xnew4[i].toFixed(6))
                // }else if(i==2){
                //     window.$arr_x3.push(window.$xnew4[i].toFixed(6))
                // }else if(i==3){
                //     window.$arr_x4.push(window.$xnew4[i].toFixed(6))
                // }
                console.log("x"+(i+1)+"="+window.$xnew4[i].toFixed(6))
...
createInputX = () => {
    ...
    componentDidMount(){
            ...
            window.$B4=[0,0,0,0]
            window.$xold4=[0,0,0,0]
            window.$xnew4=[0,0,0,0]
    }
    ...
    render()
    {
        ...
        return(
            <div className="Back">
                ...
                <div className="info3">
                    <div><button onClick={this.GaussSeidel}>submit</button></div>  
