...
class OnePointIteration extends Component
{
    constructor(props) {
      ...
      bisection = () => {
        ...
        while(i<1 ||error(xm_new,xm_old)>0.000001){
            ...
            xm_new = funtion(this.state.fx, xm_old)
            window.$arr_iteration.push(i+1)
    ...
    componentDidMount(){
            axios.get("http://192.168.99.100:8080/api/users/showOnepointiteration").then(res=>{
    ...
    //     console.log("in")
    //     await api.getOnepointiteration().then(res => {
    //         this.setState({users: res.data.data});
    //     })
    //         window.$arr_iteration = []
    //         window.$arr_x = []
